...
public class Model{
    ...
    public boolean addProduct(Product p){
        ...
        try{
        int id = this.gateway.insertProduct(p.getProductID(), p.getProdName(), p.getDescription(), p.getPrice, p.getSalePrice());
        if(id != -1){
            p.setId(id);
            this.products.add(p);
            result = true;
...
//UPDATE
    boolean updateProduct(Product p){
        ...
        try{
            ...
            Logger.getLogger(Model.class .getProdName()).log(Level.SEVERE, null, ex);
        }
        return updated;
        }
}
